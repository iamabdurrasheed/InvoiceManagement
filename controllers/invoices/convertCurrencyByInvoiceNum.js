...
module.exports = (db) => async (req, res) => {
    const { invoice_num, currency } = req.params;
    
    try {
      
      const apiUrl = `${process.env.CURRENCY_API_URL}${currency.toUpperCase()}`;
      console.log('Requesting exchange rate for invoice number:', invoice_num);
      ...
      db.get('SELECT * FROM invoices WHERE invoice_num = ?', [invoice_num], (err, invoice) => {
